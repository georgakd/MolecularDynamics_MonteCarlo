...
void Andersen(double *vx,double *vy,double *vz)
{
	 ...
	 vx[k]=sqrt((kbol*Ttarget)/m_B[k])*sqrt(-2*log(myrandom1))*cos(2*pi*myrandom2); // Box-Muller Method
	 vy[k]=sqrt((kbol*Ttarget)/m_B[k])*sqrt(-2*log(myrandom1))*sin(2*pi*myrandom2);
	 
	 myrandom1 = random_gen();
	 myrandom2 = random_gen();
	 vz[k]=sqrt((kbol*Ttarget)/m_B[k])*sqrt(-2*log(myrandom1))*cos(2*pi*myrandom2);
