...
//*********Here starts the simulated annealing or simulated quenching if chosen******************//		
	...
	{	printf("Aborting Simulation...Press 1 for exit...\n");
		exit(1);}
	
if (aneal == 1) //simulated annealing, be careful if we start from T=0, Tinitial=0
{
...
if (aneal == -1) //simulated quenching
{
	for (i=0;i<Tintervals;++i)
	{
		Tinit[i] = Ttarget-(i*TempStep);

		Tfinal[i] = Ttarget-((i+1)*TempStep);
